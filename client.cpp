...
void Client::send(const Message& message) {
    ...
    if (::send(socket_, &type, sizeof(Message::Type), 0) < 0) {
        std::cout << "Failed to send message type" << std::endl;
    }
...
void Client::update() {
    ...
    if (bytes <= 0) {
        std::cout << "Socket: message size not received: " << std::endl;
        return; // nothing to read
    }

    Message::Type type;
    if (recv(socket_, &type, sizeof(type), 0) <= 0) {
        std::cout << "Socket: message type not received" << std::endl;
        return; // nothing to read
    }

    std::vector<uint8_t> data(msgSize - sizeof(type));
    if (recv(socket_, data.data(), data.size(), 0) <= 0) {
        std::cout << "Socket: message data not received" << std::endl;
        return; // nothing to read
    }
